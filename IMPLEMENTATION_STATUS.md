...
[日本語版](docs/ja/implementation-status.md)
...
| Antenna remediation | Protect all pads, same-net GND pads/vias/traces, existing perimeter GND, and a width-`t` path to the broad core; revalidate exact residual keepouts against the current board; reject redundant or off-board copper | Requires a valid filled zone and closed Edge.Cuts for automatic residual removal; does not solve enclosure/cable excitation or radiation values |
...
| Two-layer return path | Generic transition-via warning disabled by default; checks reference gaps, detour ratio, and bottlenecks | Current distribution and frequency-dependent impedance remain approximations |
...
| Schematic-block placement | Group by sheet path, preserve locked parts, bias connectors to perimeter, place likely capacitors near matching-net pads | Dry-run planning aid, not full circuit-intent autorouting |
...
| Distribution | v0.0.2, three OS installers, no old-plugin backup creation, automatic legacy backup/staging cleanup, bilingual docs, deterministic ZIPs, CI inputs | A failed final copy requires a clean installer rerun; OS-specific Windows/macOS protection requires native acceptance |
