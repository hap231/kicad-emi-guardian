...
**EMI Guardian** is a bilingual IPC plugin for KiCad 10.0.5 and later. It combines geometry-based EMI screening, ranked ground-pour remediation, interactive board/fix/silkscreen previews, JLCPCB two-layer DFM checks, footprint-value silkscreen cleanup, and filleted board-outline proposals.
...
- Adds GitHub-ready CI, checksums, deterministic archives, Renovate dependency updates, and a bilingual Pages site.
...
See the [English user manual](docs/en/guide/user-manual.md) or [日本語取扱説明書](docs/ja/guide/user-manual.md).

The bilingual documentation site is published at [hap231.github.io/kicad-emi-guardian](https://hap231.github.io/kicad-emi-guardian/). English and Japanese pages use mirrored source paths, so CI can reject an unsynchronized documentation change.

## Distribution

Run `python tools/build_package.py` to generate the platform installers, manual-install and PCM packages, source archive, demonstration report, bilingual manuals, checksums, and machine-readable build information under `dist/`.
...
The repository includes Python 3.9/3.13/3.14 CI, strict static and documentation checks, dependency auditing, reproducible package validation, CodeQL, Renovate, and GitHub Pages deployment.

## Documentation

- [English user manual](docs/en/guide/user-manual.md) / [日本語取扱説明書](docs/ja/guide/user-manual.md)
- [JLCPCB profiles](docs/en/guide/manufacturing-jlcpcb.md) / [JLCPCBプロファイル](docs/ja/guide/manufacturing-jlcpcb.md)
- [Safety and limitations](docs/en/guide/safety-and-limitations.md) / [安全性と制限](docs/ja/guide/safety-and-limitations.md)
- [Configuration reference](docs/en/reference/configuration.md) / [設定リファレンス](docs/ja/reference/configuration.md)
- [Live acceptance test](docs/en/development/acceptance-test.md) / [実機受入試験](docs/ja/development/acceptance-test.md)
...
**EMI Guardian**は、KiCad 10.0.5以降を対象とする日英対応IPC Pluginです。青系UI、完全な日本語検出結果、拡大・移動・レイヤー切替が可能な基板／修正／シルクプレビュー、KiCad上での選択とズーム、補正済みの折れ角・電気的長配線判定、JLCPCB 2層DFM、GNDアンテナ対策、シルク整理、凸形状を基本とする外形最適化を統合します。
...
詳しい操作は[日本語取扱説明書](docs/ja/guide/user-manual.md)、製造条件は[JLCPCBプロファイル](docs/ja/guide/manufacturing-jlcpcb.md)、設定項目は[設定リファレンス](docs/ja/reference/configuration.md)を参照してください。

日英ドキュメントサイトは[hap231.github.io/kicad-emi-guardian](https://hap231.github.io/kicad-emi-guardian/)で公開されます。日英ページは同一の相対パスで管理され、片方だけを変更するとCIが検出します。
