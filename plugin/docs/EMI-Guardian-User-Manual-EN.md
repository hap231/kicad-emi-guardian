# EMI Guardian 0.0.2 User Manual
...
1. Extract `emi-guardian-0.0.2-windows-installer.zip`.
...
1. Extract `emi-guardian-0.0.2-macos-installer.zip`.
...
```bash
unzip emi-guardian-0.0.2-linux-installer.zip
cd emi-guardian-0.0.2-linux
...
Extract the `emi-guardian` directory from `emi-guardian-0.0.2-manual-install.zip` into the operating system's KiCad `plugins` directory. The final depth must be:
...
plugins/emi-guardian-0.0.2-manual-install/emi-guardian/plugin.json
```
...
Version 0.0.2 uses a blue visual system. When Japanese is selected, menus, finding titles, descriptions, recommendations, category labels, measurement labels, and JLCPCB DFM details are all presented in Japanese. Saved reports retain both Japanese and English presentation data.
...
Overall and category scores range from 0 to 100. Higher values mean fewer geometry-based concerns. Version 0.0.2 replaces unbounded linear accumulation that could pin the corner category at zero with bounded, diminishing impact. Category scores are held above 1.0.
...
Earlier versions shut down the local dashboard after a default 60-minute inactivity timer, which could look like a KiCad connection bug. Version 0.0.2 defaults to:
...
Inspect included angles. If ordinary 90-degree bends are reported, an old 0.2.x environment or cached web assets may still be running. Version 0.0.2 defaults to 75 degrees and does not flag 90 degrees.
...
Version 0.0.2 uses checkboxes. Update the plugin, recreate the managed environment, and force-refresh the browser to remove old assets.
...
## 26. Publishing on GitHub

See [Publishing the release on GitHub](github-release.md). The minimal sequence is:
...
Attach platform installers, manual/PCM/source archives, manuals, `SHA256SUMS`, and `BUILD-INFO.json` to the GitHub Release.
