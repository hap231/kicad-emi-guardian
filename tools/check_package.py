...
from project_metadata import project_version  # noqa: E402
...
def validate_pcm_metadata() -> None:
    ...
    require(latest.get("status") == "testing", "PCM status must be testing")
...
def validate_python_sources() -> None:
    ...
    paths = sorted(
        (*PLUGIN_DIR.rglob("*.py"), *(ROOT / "tools").rglob("*.py"), *(ROOT / "tests").rglob("*.py"))
    )
    for path in paths:
    ...
    for module in ("ground_connectivity.py", "stitching.py", "placement.py"):
        require((PLUGIN_DIR / "emi_guardian" / module).is_file(), f"Required module missing: {module}")


def validate_release_safety_contracts() -> None:
    """Guard installer and antenna-apply safety invariants."""
...
def validate_demo_archive(path: Path) -> None:
    """Validate the synthetic demonstration report archive."""

    require(path.is_file(), f"Demo report not found: {path}")
    root = "emi-guardian-demo-report/"
    required = {
        root + "demo-summary.json",
        root + "emi-guardian-report.html",
        root + "emi-guardian-report.json",
        root + "emi-guardian-report.md",
        root + "jlcpcb/jlcpcb-dfm-report.json",
        root + "solver/solver-manifest.json",
    }
    with zipfile.ZipFile(path) as archive:
        names = set(archive.namelist())
        require(required.issubset(names), "Demo-report ZIP is incomplete")
        for name in names:
            require(name.startswith(root), f"Unexpected demo-report ZIP root entry: {name}")
        _validate_safe_names(names)
...
def run_optional_checks() -> None:
    ...
    subprocess.run(
        [sys.executable, "-m", "compileall", "-q", "plugin", "tests", "tools"], cwd=ROOT, check=True
    )
...
def main() -> int:
    ...
    if args.archive is not None:
        ...
        elif "demo-report" in archive.name:
            validate_demo_archive(archive)
